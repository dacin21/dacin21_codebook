...
constexpr size_t pool_size = 3000000;
size_t pool_back = 0;
...
vector<ll> DP(vector<ll> const&v){
    int n = v.size();
...
signed gen(int T){
    ...
    cin >> n;
    mt19937 rng(1253636);
    auto getrand = [&](int const&l, int const&r){return uniform_int_distribution<int>(l, r)(rng);};
    ofstream o("gen_3d.txt");
    o << T << "\n";
    while(T-->0){
        o << n << "\n";
        for(int i=0;i<n;++i){
            o << getrand(-1000000, 1000000) << " ";
        }
        o << "\n";
    }
    return 0;
...
signed main_DP(){
    ...
    while(TT--){
        ...
        cerr << "t1: "<< (chrono::duration_cast<chrono::duration<double>>(t2 - t1)).count() << "\n";
        cerr << "t2: "<< (chrono::duration_cast<chrono::duration<double>>(t3 - t2)).count() << "\n";
        cerr << " ==> " << D2.back() << "\n";
        cerr << "searches: " << searches << "\n";
        cerr << "builds: " << builds << "\n";
        if(D1 == D2){
            cerr << "ok.\n";
        } else{
